/****** Object:  View [Inventory].[HealthTrackConsumption]    Script Date: 25-May-16 5:35:24 PM ******/
...
SELECT        u.invUsed_ID AS UsedId, m.Inv_ID AS ProductId, m.Inv_SPC AS SPC, m.Inv_LPC AS LPC, m.Inv_UPN AS ScanCode, m.Inv_Description AS Name,u.LOTNO as LotNumber, u.invUsed_SerialNo AS SerialNumber, 
			  u.invUsed_Qty AS Quantity, ex.RebateCode, u.invUsed_Location AS Location,l.name as LocationName, ISNULL(
						 ...
						 ,cnm.Reported,cnm.ReportedBy,cnm.ReportedOn, mrc.testDate as ConsumedOn, epm.InventoryProductId, cpc.ItemValue AS PaymentClass, cnm.ArchivedBy, cnm.ArchivedOn
...
left outer join [Inventory].[ExternalProductMapping] epm on m.Inv_ID = epm.ExternalProductId
left outer join (select top 1 map.ExternalProductId, RebateCode
	...
	where p.rebateCode is not null) ex on ex.ExternalProductId = Inv_ID
