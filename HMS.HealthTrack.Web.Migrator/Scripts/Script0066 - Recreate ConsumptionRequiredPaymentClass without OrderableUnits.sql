/****** Object:  View [Inventory].[ConsumptionRequiringPaymentClass]    Script Date: 05/05/2015 17:08:41 ******/
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[Inventory].[ConsumptionRequiringPaymentClass]'))
DROP VIEW [Inventory].[ConsumptionRequiringPaymentClass]
GO

/****** Object:  View [Inventory].[ConsumptionRequiringPaymentClass]    Script Date: 05/05/2015 17:08:42 ******/
SET ANSI_NULLS ON
GO
...
SELECT        c.ConsumptionId, c.StockId, c.ConsumedOn, c.ConsumedBy, c.DeletedOn, c.QuantityConsumed, c.Status, p.ProductId, p.SPC, p.Description, p.Manufacturer, p.PrimarySupplier, p.SecondarySupplier, 
                         p.IsConsignment, p.MinimumOrder, p.RecommendedOrder, p.OrderMultiple, p.ReorderThreshold, p.TargetStockLevel, pat.patient_ID, pat.firstName, pat.surname, pat.dob, pat.mrn, 
